...
**The dissertation describes REST as an architectural style that provides principles and constraints for building network-based applications, using the web as its foundational example.**
...
So you are probably more likely do to "RESTful" by implementing HATEOAS than by arguing about if you are allowed to use verbs or no verbs at all in your URI.

## What Fielding considers a RESTful API

In 2008 Roy Fieldings got a little frustrated: [Source](https://roy.gbiv.com/untangled/2008/rest-apis-must-be-hypertext-driven)
...
## Interpretation of the six Rules

### 1. Don't Depend on One Protocol

A REST API shouldn't be locked to one communication protocol (like HTTP). It should work as long as the protocol uses **URIs** to identify resources.
...
Your API should work with https://example.com/resource and not rely on HTTP-specific tricks.
...
You return a link in the user JSON:
...
Clients shouldn't assume or hardcode paths like /users/123/posts. Instead, they should discover URIs through links provided by the server.

Clients should learn about URIs dynamically, not hardcode them.

A client shouldn’t assume that a user’s posts are at /users/123/posts. It should read a link like this from the user resource:
...
The client shouldn’t care what kind of object a resource represents internally (like User, Admin, Moderator). It should care only about the media type (like application/json) and the links/actions it sees.
...
Build whatever makes sense for **your** project and the consumers of your API. Ignore the dogmatists who preach what RESTful APIs might be and what not. An API should be easy to learn and hard to misuse.

Who is the consumer of your API? How easy will it be for it to learn and use the API? Will it be intuitive to use? What are possible constraints? How do you version it? Deprecation and sun-setting strategies? Those things are much more valuable than the actual format of your resource identifier.
